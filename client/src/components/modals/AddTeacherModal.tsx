...
function AddTeacherModal({ courseId, onCreate }: AddTeacherModalProps) {
  ...
  const submitHandler = async () => {
    ...
    await fetch(
      `${process.env.REACT_APP_API_URL}/courses/${courseId}/teachers/${username}`,
      {
        method: "PATCH",
        headers: {
      ...
      .then(async (res) => {
        if (res.ok) {
          ...
          const teacher: User = await res.json();
      ...
      <div className="sm:max-w-lg sm:w-full m-3 sm:mx-auto">
        <div className="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto">
          <div className="flex justify-between items-center py-3 px-4 border-b">
            <h3 className="font-bold text-gray-800">Add a teacher</h3>
            <button
              type="button"
